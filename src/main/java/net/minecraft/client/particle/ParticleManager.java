...
public class ParticleManager {
	...
	public final Map<Integer, IParticleFactory> particleTypes = Maps.<Integer, IParticleFactory>newHashMap();
	...
	public void addEffect(Particle effect) {
		if (effect == null)
			return; // Forge: Prevent modders from being bad and adding nulls causing untraceable
					// NPEs.
			...
			for (int j = 0; j < 4; ++j) {
				for (int k = 0; k < 4; ++k) {
					for (int l = 0; l < 4; ++l) {
						...
						this.addEffect(
								(new ParticleDigging(this.world, (double) pos.getX() + d0, (double) pos.getY() + d1,
	...
	public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
		...
		if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE) {
			...
			this.addEffect((new ParticleDigging(this.world, d0, d1, d2, 0.0D, 0.0D, 0.0D, iblockstate)).setBlockPos(pos)
